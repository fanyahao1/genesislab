    ...
    def set_debug_vis(self, debug_vis: bool) -> bool:
        ...
        if debug_vis:
            if not hasattr(self, "origin_visualizer"):
                try:
                    from genesislab.markers import VisualizationMarkers
                    from genesislab.markers.config import FRAME_MARKER_CFG
                except ModuleNotFoundError:
                    logger.warning(
                        "Debug visualization requested for TerrainImporter, but marker utilities are unavailable."
                    )
                    return False
