...
from genesislab.utils.warp import raycast_mesh
...
def create_prim_from_mesh(prim_path: str, mesh: trimesh.Trimesh, **kwargs):
    ...
    import genesislab.sim as sim_utils
...
def find_flat_patches(
    ...
    points_np = wp_mesh.points.detach().cpu().numpy()
    x_range = (
        max(x_range[0] + origin[0].item(), points_np[:, 0].min()),
        min(x_range[1] + origin[0].item(), points_np[:, 0].max()),
    )
    y_range = (
        max(y_range[0] + origin[1].item(), points_np[:, 1].min()),
        min(y_range[1] + origin[1].item(), points_np[:, 1].max()),
    )
    z_range = (
